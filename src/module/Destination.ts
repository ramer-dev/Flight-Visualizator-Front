...
export const Destination = (map: L.Map, origin: string | LatLngExpression | null, range: number | null, distance: number | null, line = true) => {
    ...
    if (typeof origin === 'string') {
        if (origin?.includes('/')) {
            ...
            const target = L.GeometryUtil.destination(point, range * Math.PI / 180, distance * 1852)
        ...
        const target = L.GeometryUtil.destination(origin, range * Math.PI / 180, distance * 1852)
